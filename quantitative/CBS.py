...
MODEL_NAME = 'fine-tuned-llama-3.2-lora'
HUGGING_FACE_MODEL_NAME = 'meta-llama/Llama-3.2-1b'
FINE_TUNED_MODEL_PATH = 'finetuned_models/fine-tuned-llama-3.2-lora'
PROMPT_PATH = 'data/prompts/text-infilling.xlsx'
...
class Config:
    ...
    VALID_HUGGING_FACE_MODEL_NAMES = {'meta-llama/Llama-3.2-1b'}

    def __init__(self, category_type, culture_type, model_name, hugging_face_model_name, prompt_path, finetuned_model_path=None):
        ...
        self.entity_path = f"data/entities/{self.culture_type}/{self.cat_cul}.xlsx"

        probs_folder_path = f"quantitative/{model_name}/probs/"
        if not os.path.exists(probs_folder_path):
            os.makedirs(probs_folder_path)
        self.prob_path = f"quantitative/{model_name}/probs/prob_{self.cat_cul}.xlsx"


def load_model_and_tokenizer(hugging_face_model_name, fine_tuned_model_path=""):
    ...
    if fine_tuned_model_path != "":
        # adapter_model_path = "path/to/your/fine-tuned/model"  # Change this to the correct path
        model = PeftModel.from_pretrained(model, fine_tuned_model_path)

        # Merge adapter with the base model (optional, for inference)
        model = model.merge_and_unload()
        print(f"Fine-tuned model at '{fine_tuned_model_path}' loaded")
...
def get_prob_for_entities(config, model, tokenizer, device):
    ...
    for i, entity in enumerate(entities):
        ...
        for idx, token_id in enumerate(entity_ids):
            ...
            prob = torch.log(prob_distribution[:, token_id]).cpu().numpy()  # Move back to CPU for numpy operations
...
def get_average_cbs_score(categories, model_name, output_path):
    ...
    for category in categories:
        file_path = f"quantitative/{model_name}/cbs/cbs_{category.lower()}.xlsx"
        try:
            df = pd.read_excel(file_path)
...
if __name__ == '__main__':
    ...
    model, tokenizer = load_model_and_tokenizer(HUGGING_FACE_MODEL_NAME, FINE_TUNED_MODEL_PATH)
    model.to(device)
    model.eval()

    cbs_folder_path = f'quantitative/{MODEL_NAME}/cbs/'
    ...
    get_average_cbs_score(CATEGORIES, MODEL_NAME, output_path)
